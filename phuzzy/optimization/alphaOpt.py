...
class Alpha_Level_Optimization(FuzzyNumber, MPL_Mixin):
    ...
    def defuzzification(self, method = 'mean'):
        ...
        if method == 'alpha_one':
            ...
            if self.zmin_values[-1]==self.zmax_values[0]:
                ...
                Y = np.hstack((np.hstack((np.linspace(0,1,self.number_of_alpha_lvls)[:-1],np.linspace(1,0,self.number_of_alpha_lvls))),np.array([0])))
            else:
                X = np.hstack((np.hstack((self.zmin_values,self.zmax_values)),self.zmin_values[0]))
                Y = np.hstack((np.hstack((np.linspace(0,1,self.number_of_alpha_lvls),np.linspace(1,0,self.number_of_alpha_lvls))),np.array([0])))
    ...
    def _call_backup(self,iteration):
        ...
        """
        if Path('back_up').exists(): pass
        else: Path('back_up').mkdir()
        ...
        if self.start_at is None:
            ...
            df_cache = pd.DataFrame(data={'alpha': np.linspace(np.linspace(0, 1.0, self.number_of_alpha_lvls)
                                                               ...
                                                               np.linspace(0, 1.0, self.orig_number_of_alpha_lvls)
                                                               [self.orig_number_of_alpha_lvls-self.start_at],iteration+1),
                                          'l': self.zmin_values,
                                          'r': self.zmax_values})
